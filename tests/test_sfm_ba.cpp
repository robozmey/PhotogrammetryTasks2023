...
#define NIMGS_LIMIT                           15 // сколько фотографий обрабатывать (можно выставить меньше чтобы ускорить экспериментирование, или в случае если весь датасет не выравнивается)
...
//                 should_be_disabled = true;
                for (size_t cj = 0; cj < track.img_kpt_pairs.size(); ++cj) {
                    ...
                    int camera_jd = track.img_kpt_pairs[cj].first;

                    matrix3d R2; vector3d camera_origin2;
                    phg::decomposeUndistortedPMatrix(R2, camera_origin2, cameras[camera_jd]);

                    vector3d d1 = camera_origin - track_point;
                    vector3d d2 = camera_origin2 - track_point;
                    if (abs(d1.dot(d2)) < 0.1 * (cv::norm(d1) * cv::norm(d2))) {
                        should_be_disabled = true;
                        break;
                    }
                }
