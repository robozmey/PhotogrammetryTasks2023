                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (const auto & match : matches) {
        if (match.size() == 1 || match[0].distance < match[1].distance * 0.5)
            filtered_matches.push_back(match[0]);
    }

    std::cout << filtered_matches.size() << std::endl;
}
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    std::shared_ptr<cv::flann::IndexParams> index_params = flannKdTreeIndexParams(5);
    ...
    for (int i = 0; i < n_matches; i++) {
        int current_consistent_matches = 0;

        std::set<int> train_neighbour_matches_indices;
        for (int j = 0; j < total_neighbours; j++) {
            if (radius2_train >= distances2_query.at<float>(i, j)) {
                train_neighbour_matches_indices.insert(indices_train.at<int>(i, j));
            }
        }

        for (int j = 0; j < total_neighbours; j++) {
            if (radius2_query >= distances2_train.at<float>(i, j)) {
                if (train_neighbour_matches_indices.count(indices_query.at<int>(i, j))) {
                    current_consistent_matches++;
                }
            }
        }

        if (current_consistent_matches >= consistent_matches)
